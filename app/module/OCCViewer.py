...
from OCC.Core.Aspect import Aspect_TOTP_RIGHT_LOWER, Aspect_FM_STRETCH, Aspect_FM_NONE,Aspect_TOTP_LEFT_LOWER
...
class Viewer3d(Display3d):
        ...
        for shape in shapes:
            if material or texture:
                if texture:
                    ...
                    #shape_to_display.SetDisplayMode(0)
                elif material:
    ...
    def DisplayShape_stl(self, shapes, material=Graphic3d_NOM_STEEL, texture=None, color=None, transparency=None, update=False):
        """ display one or a set of displayable objects
        """
        ais_shapes = []  # the list of all displayed shapes
        ...
        for shape in shapes:
            ...
            if material or texture:
                if texture:
                    ...
                    shape_to_display.SetTextureRepeat(True, toRepeatU, toRepeatV)
                    shape_to_display.SetTextureOrigin(True, originU, originV)
                    shape_to_display.SetDisplayMode(1)
                elif material:
                    shape_to_display = AIS_Shape(shape)
