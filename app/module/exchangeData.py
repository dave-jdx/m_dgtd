...
from OCC.Core.XCAFDoc import (XCAFDoc_DocumentTool_ShapeTool,
                              XCAFDoc_DocumentTool_ColorTool, XCAFDoc_DocumentTool_LayerTool)
...
def write_step_file_named(shapeList,filename,named=(None,None),application_protocol="AP203"):
     # a few checks
    # if a_shape.IsNull():
    ...
    faceDic:dict=named[0]
    bodyDic:dict=named[1]
    # 创建一个文档来存储形状及其元数据
    app = XCAFApp_Application.GetApplication()
    doc = TDocStd_Document(TCollection_ExtendedString("PythonOCC_Document"))
    ...
    for shape in shapeList:
        ...
        for solid in solidList:
            ...
            shape_tool.SetShape(shape_label, solid)
            if(bodyDic.get(solidIndex)!=None):
                v=bodyDic[solidIndex]
                TDataStd_Name.Set(shape_label, TCollection_ExtendedString("Medium"+str(v)))
            else:
                TDataStd_Name.Set(shape_label, TCollection_ExtendedString(""))
            solidIndex+=1
        face_explorer = TopExp_Explorer(shape, TopAbs_FACE)
        while face_explorer.More():
            face=face_explorer.Current()
            # 为面创建新的标签
            # face_label = shape_tool.NewShape()
            
            # 将面单独添加为一个独立的形状标签
       
            if(faceDic.get(faceIndex)!=None):
                face_label=shape_tool.AddShape(face)
                TDataStd_Name.Set(face_label, TCollection_ExtendedString("Bound"+str(faceIndex)))
...
def read_step_file_with_names_colors(filename,self=None):
    """ Returns list of tuples (topods_shape, label, color)
    Use OCAF.
    """
    # # self.parent.PartLoadProgressBar.progressBar.show()
    # self.parent.PartLoadProgressBar.label.show()
    # self.parent.PartLoadProgressBar.label.setText("数据读取中:")

    # step_reader = STEPControl_Reader()
    # status = step_reader.ReadFile("example.stp")
    ...
    output_shapes = {}
    #设置进度条值
    # # self.parent.PartLoadProgressBar.progressBar.setValue(20)
    ...
    # 设置进度条值
    # # self.parent.PartLoadProgressBar.progressBar.setValue(30)
    print("读取文件",filename)
    status = step_reader.ReadFile(filename)
    print("读取文件",status)
    ...
    # # self.parent.PartLoadProgressBar.progressBar.setValue(45)
    def _get_sub_shapes(lab, loc):
        ...
        #print("Is IsTopLevel   :", shape_tool.IsTopLevel(lab))
        #print("DumpToString   :", shape_tool.DumpToString())#��ȡװ���ϵ
        ...
        l_freeshape=TDF_LabelSequence()
        #a123=shape_tool.GetFreeShapes(l_freeshape)
    ...
    _get_shapes()
    # 设置进度条值
    # # self.parent.PartLoadProgressBar.progressBar.setValue(50)
    print("{0} shapes read".format(len(output_shapes)))
    return output_shapes,assemble_relation_list,DumpToString
...
#########################
def write_stl_file(a_shape, filename, mode="ascii", linear_deflection=1000, angular_deflection=0.5):
...
def read_stl_file(filename,self=None):
    ...
    # reader.Read(the_shape,filename)
    try:
        stlapi_Read(the_shape, filename)
        # self.parent.PartLoadProgressBar.progressBar.setValue(40)  # 进度条
        if the_shape.IsNull():
            raise AssertionError("Shape is null.")

        # self.import_shape=the_shape
        end_time = time.time()
        print("reader.stl耗时: {:.2f}秒".format(end_time - start_time))
        return the_shape
...
def read_iges_file(filename,return_as_shapes=False, verbosity=False, visible_only=False):
    ...
    _shapes = []
    # self.parent.PartLoadProgressBar.progressBar.setValue(20)  # 进度条
    ...
    if status == IFSelect_RetDone:  # check status
        ...
        # self.parent.PartLoadProgressBar.progressBar.setValue(30)  # 进度条
        # self.AppRefrsh()
...
def analyse(assemble_relation_list):#��ȡÿ�����װ������
    new_assemble_relation_list={}
    for i in (assemble_relation_list):
        new_assemble_relation_list[i["name"]]=i["components number"]
    return new_assemble_relation_list

def Get_assemble_relation(DumpToString="",assemble_relation_list=""):#��ȡ����part װ���ϵ
     assemble_list=(re.findall(r'"(.*?)"', DumpToString))
     assemble_list.pop()
     assemble_struct_list=[]

     for i in assemble_list:
         assemble = []
         try:
            part_name=i[0:len(i)-2]
            num = assemble_relation_list[part_name]
         except:
             part_name = i
             num = assemble_relation_list[part_name]
         #print(i,num,assemble_list)
         if num!=0:
            for j in range(assemble_list.index(i),num+1+assemble_list.index(i)):
                assemble.append(assemble_list[j])
            for j in range(num):
                assemble_list.remove(assemble_list[0])
         elif num==0:
            continue
         assemble_struct_list.append(assemble)
     return  assemble_struct_list
...
if __name__=="__main__":
    pass
    shapes_labels_colors,assemble_relation_list,DumpToString= read_step_file_with_names_colors("../examples/assets/models/as1-oc-214.stp")
    display, start_display, add_menu, add_function_to_menu = init_display()
    for shpt_lbl_color in shapes_labels_colors:
        label, c,dict1 = shapes_labels_colors[shpt_lbl_color]
        display.DisplayColoredShape(shpt_lbl_color, color=Quantity_Color(c.Red(),
                                                                         c.Green(),
                                                                         c.Blue(),
                                                                         Quantity_TOC_RGB))
    new_assemble_relation_list=analyse(assemble_relation_list)
    Get_assemble_relation(DumpToString,new_assemble_relation_list)
